...
.sidebar-mini .menu-text{display:none}
.sidebar-mini nav a{justify-content:center}
.sidebar-mini .material-icons-outlined{margin-right:0!important}
.content-mini{margin-left:72px!important}
.menu-active{background:#fff;font-weight:600}
...
<span class="inbox-badge bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs">
<?= $inbox_count ?>
</span>
<?php endif; ?>
</a>
...
<button id="prevBtn"
class="px-4 py-2 bg-slate-200 rounded hover:bg-slate-300 disabled:opacity-40">
Prev
</button>
...
toggleBtn.onclick=()=>{sidebar.classList.toggle('sidebar-mini');content.classList.toggle('content-mini')};
...
rowsPerPageSelect.onchange=()=>{rowsPerPage=parseInt(rowsPerPageSelect.value);currentPage=1;renderTable();}
prevBtn.onclick=()=>{if(currentPage>1){currentPage--;renderTable();}}
nextBtn.onclick=()=>{if(currentPage<Math.ceil(rows.length/rowsPerPage)){currentPage++;renderTable();}}
